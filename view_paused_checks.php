...
$consulta=mysql_query("SELECT * FROM servers where inactive = 0");
...
while ($dados = mysql_fetch_array($consulta)) {
	$id = $dados["id"];
	$consulta_dash=mysql_query("SELECT id,dns_host,dns_target,dns_ip_target,dns_timestamp,count,last_check,scanned,pause FROM dns_checks where id_server = $id and pause=1");
